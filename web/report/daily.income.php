...
<script>
    function createPDFDailyIncome(date, data) {
        var dataTable = "";
		var amountTotal = 0;
		var discountTotal = 0;
		var balanceTotal = 0;
        ...
        win.document.write('<title>Daily Income Report</title>');
        win.document.write(style);
        win.document.write('</head>');
        win.document.write('<body>');
        win.document.write('<h1>Daily Income Report : '+String(date)+'</h1>');
	...
	function callJsonPDFDailyIncome(date) {
	    var formData = '{ "status" : "PAID", "date" : "'+date+'" }';
	            ...
	            if((xhr.status==200) && (status=="success")){
	              if(response["error"]==false){
	              	if(parseInt(response["result"]["count"])!=0){
	                	createPDFDailyIncome(date, response["result"]["data"]);
	...
	function callJsonPDFDailyForecastIncome(date) {
	    var formData = '{ "status" : "DELIVERY", "date" : "'+date+'" }';
	    //console.log(formData);
	    var jsonData = jQuery.parseJSON(formData);
	    //console.log(jsonData);
	    //console.log('<?=$_DOMAIN?>api/json/viewReportOrder/');
